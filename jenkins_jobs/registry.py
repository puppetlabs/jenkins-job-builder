...
class MacroRegistry(object):
    ...
    __component_to_component_list_mapping = {}
    __component_list_to_component_mapping = {}
    __macros_by_component_type = {}
    __macros_by_component_list_type = {}
    ...
    def __init__(self):
        ...
        for entrypoint in pkg_resources.iter_entry_points(
            ...
            Mod = entrypoint.load()
            self.__component_list_to_component_mapping[
                Mod.component_list_type] = Mod.component_type
            self.__component_to_component_list_mapping[
                Mod.component_type] = Mod.component_list_type
            self.__macros_by_component_type[
                Mod.component_type] = {}
            self.__macros_by_component_list_type[
                Mod.component_list_type] = {}

        self.__mask_warned = {}

    @property
    def __nonempty_component_list_types(self):
        return [clt for clt in self.__macros_by_component_list_type
                if len(self.__macros_by_component_list_type[clt]) != 0]

    @property
    def component_types(self):
        return self.__macros_by_component_type.keys()

    def __is_macro(self, component_name, component_list_type):
        return (component_name in
                self.__macros_by_component_list_type[component_list_type])

    def register(self, component_type, macro):
        macro_name = macro["name"]
        clt = self.__component_to_component_list_mapping[component_type]
        self.__macros_by_component_type[component_type][macro_name] = macro
        self.__macros_by_component_list_type[clt][macro_name] = macro
    ...
    def expand_macros(self, jobish, template_data=None):
        ...
        variables.
        """
        for component_list_type in self.__nonempty_component_list_types:
            self.__expand_macros_for_component_list_type(
                jobish, component_list_type, template_data)

    def __expand_macros_for_component_list_type(self,
                                                jobish,
                                                component_list_type,
                                                template_data=None):
        ...
        for component in component_list:
            macro_component_list = self.__maybe_expand_macro(
    ...
    def __maybe_expand_macro(self,
        ...
        if isinstance(component_copy, dict):
        ...
        # macro.
        if not self.__is_macro(component_name, component_list_type):
            return None
        ...
        if ModuleRegistry.is_module_name(component_name, component_list_type):
            self.__mask_warned[component_name] = True
            logger.warning(
        ...
        macro_component_list = self.__get_macro_components(component_name,
                                                           component_list_type)
        ...
        if component_data:
            ...
            if template_data:
                ...
                macro_component_list = deep_format(
                    macro_component_list, template_data, True)
            else:
                macro_component_list = deep_format(
                    macro_component_list, component_data, True)

        return macro_component_list

    def __get_macro_components(self, macro_name, component_list_type):
...
class ModuleRegistry(object):
    ...
    def dispatch(self, component_type, xml_parent, component):
        ...
        eps = ModuleRegistry.__entry_points_cache.get(component_list_type)
        if eps is None:
            module_eps = []
            ...
            ModuleRegistry.__entry_points_cache[component_list_type] = eps
