...
def xml_elements_tostring(node):
    ...
    It would happen that some node.text were loaded with other types like bool
    or list, which at this point of xml generation errors out with library XML
    methods like dump() or tostring()

    This should really be dealt with at the xml creation step & ensure valid
    data, but I cannot go over it all so I'm lazily adding a default sanity
    method.

    This may create 'dirty' strings for example save a list as its string
    representation in python e.g. ['one', 'two', 'three']
